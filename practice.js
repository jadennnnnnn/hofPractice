...
var sumTotal = function(products) {
  return _.reduce(products, function(total, product){
    return total + Number(product.price.substring(1));
  }, 0);
};
...
var dessertCategories = function(desserts) {
  return _.reduce(desserts, function(total, dessert){
    total[dessert.type]++ || (total[dessert.type] = 1);
    return total;
  }, {});
};
...
var applyCoupon = function(groceries, coupon) {
  return _.map(groceries, function(grocery){
    var price = Number(grocery.price.substring(1));
    var discount = (price * coupon).toFixed(2);
    grocery.salePrice = '$' + (price - discount);
    return grocery;
  });
};
